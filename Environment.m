%% Environment 

%% Floor https://au.mathworks.com/matlabcentral/answers/300826-how-do-i-insert-an-image-in-my-2-d-and-3-d-plots-in-matlab-8-2-r2013b
axis equal;   % Make the axes scales match
hold on;      % Add to the plot
xlabel('x');
ylabel('y');
zlabel('z');
img = imread('grass.jpg');     % Load a sample image
yImage = [-1.5 1.5; -1.5 1.5];   % The x data for the image corners
zImage = [-0.39 -0.39; -0.39 -0.39];             % The y data for the image corners
xImage = [1.5 1.5; -1.5 -1.5];   % The z data for the image corners
surf(xImage,yImage,zImage,...    % Plot the surface
     'CData',img,...
     'FaceColor','texturemap'); 
%% TABLE
[f,v,data] = plyread('stand.ply','tri');
vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;

        tableMesh_h = trisurf(f,v(:,1) + 0,v(:,2) + 0, v(:,3) - 0.08 ...
        ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');

hold on; 

%% Fences
% 
%         %Fence 1
%         [f,a,data] = plyread('fence.ply','tri');
%         vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
%         fence_h = trisurf(f,a(:,1) + 0,a(:,2) -1, a(:,3) + 0 ...
%             ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
%         hold on;
%         
%         %Fence 2
%         [f,a,data] = plyread('fence.ply','tri');
%         vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
%         fence_h = trisurf(f,a(:,1) + 0,a(:,2) + 1, a(:,3) -0 ...
%             ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
%         hold on;
        
        %Fence 3
        [f,a,data] = plyread('fence1.ply','tri');
        vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
        fence_h = trisurf(f,a(:,1) - 1.2,a(:,2) + 0, a(:,3) - 0 ...
            ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
        hold on;
        
        %Fence 4
        [f,a,data] = plyread('fence1.ply','tri');
        vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
        fence_h = trisurf(f,a(:,1) + 1.2,a(:,2) + 0, a(:,3) -0 ...
            ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
        hold on;
        
%% Bricks
        %Brick Wall
        [f,a,data] = plyread('brickwall.ply','tri');
        vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
        fence_h = trisurf(f,a(:,1) + 0.2,a(:,2) + 0, a(:,3) -0.4 ...
            ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
        hold on;
        
        %Brick Stack
        [f,a,data] = plyread('brickstack.ply','tri');
        vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
        fence_h = trisurf(f,a(:,1) + 0,a(:,2) - 0.2, a(:,3) -0.4 ...
            ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat');
        hold on;
        
        
        
        %Brick 
        [f,b,data] = plyread('brick.ply','tri');
        vertexColours = [data.vertex.red, data.vertex.green, data.vertex.blue] / 255;
        Brick_h = trisurf(f,b(:,1) + 0,b(:,2) - 0.2, b(:,3) + 0.1 ...
            ,'FaceVertexCData',vertexColours,'EdgeColor','interp','EdgeLighting','flat')
        
        brickVertexCount = size(b,1);
        BrickPose = eye(6);
        hold on; 