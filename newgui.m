...
%
%      NEWGUI('Property','Value',...) creates a new NEWGUI or raises
...
% handles    structure with handles and user data (see GUIDATA)
 ...
 handles.cyton.pickBrick(0,-0.2,0.1)
 handles.cyton.dropBrick(0.2,0,0.25,1,handles.cyton.brickVertexCount,handles.cyton.b);
 handles.cyton.dropBrick(0.27,0.05,0.09,1,handles.cyton.brickVertexCount,handles.cyton.b);
 handles.cyton.pickBrick(0.2,0,0.25)
 handles.cyton.pickBrick(0.041,-0.2,0.1)
 handles.cyton.dropBrick(0.2,0,0.25,2,handles.cyton.brickVertexCount1,handles.cyton.b1);
 handles.cyton.dropBrick(0.27,-0.04,0.09,2,handles.cyton.brickVertexCount1,handles.cyton.b1);
 handles.cyton.pickBrick(0.2,0,0.25)
 handles.cyton.pickBrick(-0.041,-0.2,0.1)
 handles.cyton.dropBrick(0.1,-0.2,0.3,3,handles.cyton.brickVertexCount2,handles.cyton.b2);
 handles.cyton.dropBrick(0.25,-0.09,0.085,3,handles.cyton.brickVertexCount2,handles.cyton.b2);
...
newQ=get(hObject,'Value');
q1 = handles.cyton.model.getpos();
q2= q1;
q2(1,6)=q2(1,6)+newQ;
handles.cyton.model.plot(q2);
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
v =  get(hObject,'Value')
newPos = handles.cyton.model.fkine(handles.cyton.model.getpos())*transl(0,0,v);
handles.cyton.pickBrick(newPos(1,4),newPos(2,4),newPos(3,4))
set(hObject,'Min',0)
set(hObject,'Max',0.03)
...
% handles    structure with handles and user data (see GUIDATA)
v =  get(hObject,'Value')
newPos = handles.cyton.model.fkine(handles.cyton.model.getpos())*transl(v,0,0);
handles.cyton.pickBrick(newPos(1,4),newPos(2,4),newPos(3,4))
set(hObject,'Min',-0.075)
set(hObject,'Max',0.075)
...
%        get(hObject,'Min') and get(hObject,'Max') to determine range of slider
v =  get(hObject,'Value')
newPos = handles.cyton.model.fkine(handles.cyton.model.getpos())*transl(0,v,0);
handles.cyton.pickBrick(newPos(1,4),newPos(2,4),newPos(3,4))
set(hObject,'Min',-0.075)
set(hObject,'Max',0.075)
...
triggerconfig (handles.depthVid,'manual');
framesPerTrig = 1;
handles.depthVid.FramesPerTrigger=framesPerTrig;
handles.depthVid.TriggerRepeat=inf;
src = getselectedsource(handles.depthVid);
src.EnableBodyTracking = 'on'; 
% triggerconfig([handles.depthVid colorVid],'manual');
start(handles.depthVid);
...
while ishandle(himg)
trigger(handles.depthVid)
[depthMap, ts, depthMetaData] = getdata (handles.depthVid);
...
if handPos(1,1) > 0.3 && handles.cyton.taskExecutionFlag==0
    
    handles.cyton.pickBrick1();
end

if handPos(1,3)<1 && handles.cyton.taskExecutionFlag==1
   
    handles.cyton.DropBrick1();
end
% 
if handPos(1,1)>0.3 && handles.cyton.taskExecutionFlag==2
    
    handles.cyton.pickBrick2();
end
% 
if handPos(1,3)<1 && handles.cyton.taskExecutionFlag==3
    
    handles.cyton.DropBrick2();
end
% 
if handPos(1,1)>0.3 && handles.cyton.taskExecutionFlag==4
    
    handles.cyton.pickBrick3();
end

if handPos(1,3)<1 && handles.cyton.taskExecutionFlag==5
    
    handles.cyton.DropBrick3();
    
end
if handles.cyton.taskExecutionFlag==6
   disp('task executed') 
   break;
end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles.cyton.pickBrick(0.27,0.05,0)
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
hObject = handles.cyton.model.fkine(handles.cyton.model.getpose());
